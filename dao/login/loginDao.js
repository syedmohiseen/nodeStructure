...
loginDao.prototype.getLoggedInUserDetails = function(data,cb){
    var userData = data.body;
    var connection = baseDao.getConnection(data);
    var query = 'select * from admin WHERE firstName= "'+userData.firstName+'" AND password="'+userData.password+'"';
  connection.query(query, function(err,result){
     if(err){
            return cb(message.internalServerError,httpResponseCodes.INTERNAL_SERVER_ERROR);
        }
...
loginDao.prototype.addUser = function(data,cb){
    var userData = data.body;
    var connection = baseDao.getConnection(data);
    var query = 'INSERT INTO admin (userId, LastName, FirstName, mailId,password) VALUES("'+userData.userId+'","'+userData.userId+'","'+userData.FirstName+'","'+userData.mailId+'","'+userData.password+'")';
  connection.query(query, function(err,result){
     if(err){
            return cb(message.internalServerError,httpResponseCodes.INTERNAL_SERVER_ERROR);
        }
